...
The weighted average distance function is called `zsum` in the package, given a point, for example: $(3, 1)$ you can evaluate the weighted average distance to a `loca.p` object:
```{r}
pt3 <- loca.p (x = c (0, 4, 2), y = c (0, 0, 2), label = "Three points")
zsum (o = pt3, x = 3, y = 1)
```
You can also calculate the gradient of `zsum` called `zsumgra`:
```{r}
...
To find the optimal solution to the previous location problem, use the `zsummin` function:
```{r}
s <- zsummin (pt3)
s
```
